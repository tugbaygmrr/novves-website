/**
 * tr/*.json → tg/*.json (структураи JSON нигоҳ дошта мешавад, матн ба тоҷикӣ).
 *
 * Истифода:
 *   node scripts/i18n-tr-to-tg.mjs
 *   node scripts/i18n-tr-to-tg.mjs --file solutions --fresh
 *   node scripts/i18n-tr-to-tg.mjs --file solutions --keys dumanIsiTahliye,konforIklimlendirme
 *
 * Муҳит:
 *   I18N_TR_TO_TG_ENGINE=gtx|mymemory|google|both   (пешфарз: gtx)
 *   I18N_TR_TO_TG_GTX_DELAY_MS, I18N_TR_TO_TG_GTX_MAX (~1200)
 *   I18N_TR_TO_TG_DELAY_MS, I18N_TR_TO_TG_COOLDOWN_MS — барои vitalets google
 *   I18N_TR_TO_TG_MM_DELAY_MS — MyMemory
 *
 * npm: npm run i18n:tr-to-tg
 */
import fs from "fs";
...
const ENGINE = (process.env.I18N_TR_TO_TG_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_TR_TO_TG_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_TR_TO_TG_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_TR_TO_TG_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_TR_TO_TG_GTX_DELAY_MS) || 380;
const GTX_MAX = Number(process.env.I18N_TR_TO_TG_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
...
function checkpointPathFor(tgPath) {
  return tgPath.replace(/\.json$/i, "") + ".tr-to-tg.checkpoint.json";
}
...
async function translateGtxOne(chunk) {
  let lastErr;
  const urlBase =
    "https://translate.googleapis.com/translate_a/single?client=gtx&sl=tr&tl=tg&dt=t&q=";
...
async function translateMyMemoryOne(chunk) {
  ...
  const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(chunk)}&langpair=tr|tg`;
...
async function translateGoogleStr(str, cache) {
  ...
  for (let attempt = 0; attempt < RETRIES; attempt++) {
    ...
    await sleep(wait);
    try {
      const res = await translate(str, { from: "tr", to: "tg" });
...
async function translateText(str, cache) {
  ...
  try {
    ...
    if (ENGINE === "gtx") {
      ...
      console.error("Unknown I18N_TR_TO_TG_ENGINE:", ENGINE, "— use gtx|mymemory|google|both");
      out = await translateGtxStr(str);
    }
    cache.set(str, out);
    return out;
...
async function processFile(name) {
  ...
  const tgPath = path.join(DICT, "tg", name);
  const cpPath = checkpointPathFor(tgPath);
  ...
  let tgObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(tgPath)) {
    try {
      tgObj = JSON.parse(fs.readFileSync(tgPath, "utf8"));
    } catch {
      tgObj = {};
    }
  }
  ...
  for (const key of topKeys) {
    ...
    tgObj[key] = await mapLeaves(trObj[key], cache);
    fs.writeFileSync(tgPath, JSON.stringify(tgObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", tgPath, "after", key, "| cache size:", cache.size);
  }
