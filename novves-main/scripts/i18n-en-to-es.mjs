/**
 * en/*.json → es/*.json (JSON structure preserved, leaf strings → Spanish).
 *
 * Usage:
 *   node scripts/i18n-en-to-es.mjs
 *   node scripts/i18n-en-to-es.mjs --file solutions --fresh
 *   node scripts/i18n-en-to-es.mjs --file solutions --keys dumanIsiTahliye
 *
 * Env:
 *   I18N_EN_TO_ES_ENGINE=gtx|mymemory|google|both   (default: gtx)
 *   I18N_EN_TO_ES_GTX_DELAY_MS, I18N_EN_TO_ES_GTX_MAX (~1200)
 *   I18N_EN_TO_ES_DELAY_MS, I18N_EN_TO_ES_COOLDOWN_MS — vitalets google
 *   I18N_EN_TO_ES_MM_DELAY_MS — MyMemory
 *
 * npm: npm run i18n:en-to-es
 */
import fs from "fs";
...
const ENGINE = (process.env.I18N_EN_TO_ES_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_EN_TO_ES_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_EN_TO_ES_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_EN_TO_ES_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_EN_TO_ES_GTX_DELAY_MS) || 320;
const GTX_MAX = Number(process.env.I18N_EN_TO_ES_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
...
function checkpointPathFor(esPath) {
  return esPath.replace(/\.json$/i, "") + ".en-to-es.checkpoint.json";
}
...
async function translateGoogleStr(str, cache) {
  ...
  for (let attempt = 0; attempt < RETRIES; attempt++) {
    ...
    await sleep(wait);
    try {
      const res = await translate(str, { from: "en", to: "es" });
...
async function translateText(str, cache) {
  ...
  try {
    ...
    if (ENGINE === "gtx") {
      ...
      console.error("Unknown I18N_EN_TO_ES_ENGINE:", ENGINE, "— use gtx|mymemory|google|both");
      out = await translateGtxStr(str);
    }
    cache.set(str, out);
    return out;
...
async function processFile(name) {
  ...
  const esPath = path.join(DICT, "es", name);
  const cpPath = checkpointPathFor(esPath);
  ...
  let esObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(esPath)) {
    try {
      esObj = JSON.parse(fs.readFileSync(esPath, "utf8"));
    } catch {
      esObj = {};
    }
  }
  ...
  for (const key of topKeys) {
    ...
    esObj[key] = await mapLeaves(enObj[key], cache);
    fs.writeFileSync(esPath, JSON.stringify(esObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", esPath, "after", key, "| cache size:", cache.size);
  }
