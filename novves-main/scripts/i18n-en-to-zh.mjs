/**
 * en/*.json → zh/*.json (JSON structure preserved, leaf strings → Simplified Chinese).
 *
 * Usage:
 *   node scripts/i18n-en-to-zh.mjs
 *   node scripts/i18n-en-to-zh.mjs --file solutions --fresh
 *
 * Env:
 *   I18N_EN_TO_ZH_ENGINE=gtx|mymemory|google|both   (default: gtx)
 *   I18N_EN_TO_ZH_GTX_DELAY_MS, I18N_EN_TO_ZH_GTX_MAX (~1200)
 *   I18N_EN_TO_ZH_DELAY_MS, I18N_EN_TO_ZH_COOLDOWN_MS
 *   I18N_EN_TO_ZH_MM_DELAY_MS
 *
 * npm: npm run i18n:en-to-zh
 */
import fs from "fs";
...
const ENGINE = (process.env.I18N_EN_TO_ZH_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_EN_TO_ZH_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_EN_TO_ZH_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_EN_TO_ZH_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_EN_TO_ZH_GTX_DELAY_MS) || 320;
const GTX_MAX = Number(process.env.I18N_EN_TO_ZH_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
const MYMEMORY_QUOTA_ERR = "MYMEMORY_QUOTA_EXCEEDED";
const RETRIES = 12;

/** GTX / MyMemory target (Simplified Chinese) */
const TL = "zh-CN";
...
function checkpointPathFor(zhPath) {
  return zhPath.replace(/\.json$/i, "") + ".en-to-zh.checkpoint.json";
}
...
async function translateGoogleStr(str, cache) {
  ...
  for (let attempt = 0; attempt < RETRIES; attempt++) {
    ...
    await sleep(wait);
    try {
      const res = await translate(str, { from: "en", to: "zh-CN" });
...
async function translateText(str, cache) {
  ...
  try {
    ...
    if (ENGINE === "gtx") {
      ...
      console.error("Unknown I18N_EN_TO_ZH_ENGINE:", ENGINE, "— use gtx|mymemory|google|both");
      out = await translateGtxStr(str);
    }
    cache.set(str, out);
    return out;
...
async function processFile(name) {
  ...
  const zhPath = path.join(DICT, "zh", name);
  const cpPath = checkpointPathFor(zhPath);
  ...
  let zhObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(zhPath)) {
    try {
      zhObj = JSON.parse(fs.readFileSync(zhPath, "utf8"));
    } catch {
      zhObj = {};
    }
  }
  ...
  for (const key of topKeys) {
    ...
    zhObj[key] = await mapLeaves(enObj[key], cache);
    fs.writeFileSync(zhPath, JSON.stringify(zhObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", zhPath, "after", key, "| cache size:", cache.size);
  }
