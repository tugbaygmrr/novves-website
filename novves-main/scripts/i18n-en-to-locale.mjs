/**
 * en/*.json → {locale}/*.json (generic en→target locale translator).
 *
 * Usage:
 *   node scripts/i18n-en-to-locale.mjs --locale ru
 *   node scripts/i18n-en-to-locale.mjs --locale ru,de,fr --file technical --fresh
 *
 * Env: I18N_EN_TO_LOCALE_ENGINE, I18N_EN_TO_LOCALE_GTX_DELAY_MS (same pattern as en-to-pl)
 */
import fs from "fs";
...
const ENGINE = (process.env.I18N_EN_TO_LOCALE_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_EN_TO_LOCALE_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_EN_TO_LOCALE_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_EN_TO_LOCALE_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_EN_TO_LOCALE_GTX_DELAY_MS) || 320;
const GTX_MAX = Number(process.env.I18N_EN_TO_LOCALE_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
...
if (locIdx < 0 || !args[locIdx + 1]) {
  console.error("Usage: node scripts/i18n-en-to-locale.mjs --locale ru[,de,...] [--file technical] [--fresh]");
  process.exit(1);
}
const TARGET_LOCALES = args[locIdx + 1]
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const DEFAULT_FILES = ["technical.json"];
...
function checkpointPathFor(targetPath) {
  return targetPath.replace(/\.json$/i, "") + ".en-to-locale.checkpoint.json";
}
...
function makeTranslator(tl) {
  ...
  async function translateGtxStr(str) {
    ...
    const out = [];
    for (const p of parts) out.push(await translateGtxOne(p));
    return out.join("\n\n");
  }

  async function translateMyMemoryOne(chunk) {
    await sleep(MYMEMORY_DELAY_MS);
    const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(chunk)}&langpair=en|${tl}`;
  ...
  async function translateMyMemoryStr(str) {
    ...
    const out = [];
    for (const p of parts) out.push(await translateMyMemoryOne(p));
    return out.join("\n\n");
  }
  ...
  async function translateGoogleStr(str, cache) {
    ...
    for (let attempt = 0; attempt < RETRIES; attempt++) {
      ...
      await sleep(wait);
      try {
        const res = await translate(str, { from: "en", to: tl });
  ...
  return async function translateText(str, cache) {
...
async function mapLeaves(obj, translateText, cache) {
  if (typeof obj === "string") return translateText(obj, cache);
  if (Array.isArray(obj)) {
    const out = [];
    for (const item of obj) out.push(await mapLeaves(item, translateText, cache));
    return out;
  }
  if (obj !== null && typeof obj === "object") {
    const out = {};
    for (const k of Object.keys(obj)) {
      out[k] = await mapLeaves(obj[k], translateText, cache);
    }
    return out;
  }
  return obj;
...
async function processFile(locale, name) {
  const tl = LOCALE_TL[locale];
  if (!tl) {
    console.error("Unsupported locale:", locale, "— supported:", Object.keys(LOCALE_TL).join(", "));
    return;
  }

  const translateText = makeTranslator(tl);
  const enPath = path.join(DICT, "en", name);
  const targetPath = path.join(DICT, locale, name);
  const cpPath = checkpointPathFor(targetPath);
  ...
  console.error("Translating", locale, name, "engine=", ENGINE, ONLY_KEYS ? `--keys ${[...ONLY_KEYS].join(",")}` : "…");
  const enObj = JSON.parse(fs.readFileSync(enPath, "utf8"));
  const cache = new Map();

  let targetObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(targetPath)) {
    try {
  ...
  for (const key of topKeys) {
    ...
    console.error("Translating section:", locale, key, "…");
    targetObj[key] = await mapLeaves(enObj[key], translateText, cache);
    fs.writeFileSync(targetPath, JSON.stringify(targetObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", targetPath, "after", key, "| cache size:", cache.size);
  }

  console.error("Finished", locale, name, "unique strings cached:", cache.size);
}
