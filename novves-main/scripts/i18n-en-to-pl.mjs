/**
 * en/*.json → pl/*.json (JSON structure preserved, leaf strings → Polish).
 *
 * Usage:
 *   node scripts/i18n-en-to-pl.mjs
 *   node scripts/i18n-en-to-pl.mjs --file solutions --fresh
 *
 * Env:
 *   I18N_EN_TO_PL_ENGINE=gtx|mymemory|google|both   (default: gtx)
 *   I18N_EN_TO_PL_GTX_DELAY_MS, I18N_EN_TO_PL_GTX_MAX (~1200)
 *   I18N_EN_TO_PL_DELAY_MS, I18N_EN_TO_PL_COOLDOWN_MS
 *   I18N_EN_TO_PL_MM_DELAY_MS
 *
 * npm: npm run i18n:en-to-pl
 *
 * If `pl` looks wrong or stale: use `npm run i18n:en-to-pl -- --fresh` to rebuild all sections
...
const ENGINE = (process.env.I18N_EN_TO_PL_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_EN_TO_PL_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_EN_TO_PL_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_EN_TO_PL_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_EN_TO_PL_GTX_DELAY_MS) || 320;
const GTX_MAX = Number(process.env.I18N_EN_TO_PL_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
...
function checkpointPathFor(plPath) {
  return plPath.replace(/\.json$/i, "") + ".en-to-pl.checkpoint.json";
}
...
async function translateMyMemoryOne(chunk) {
  ...
  const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(chunk)}&langpair=en|pl`;
...
async function translateGoogleStr(str, cache) {
  ...
  for (let attempt = 0; attempt < RETRIES; attempt++) {
    ...
    await sleep(wait);
    try {
      const res = await translate(str, { from: "en", to: "pl" });
...
async function translateText(str, cache) {
  ...
  try {
    ...
    if (ENGINE === "gtx") {
      ...
      console.error("Unknown I18N_EN_TO_PL_ENGINE:", ENGINE, "— use gtx|mymemory|google|both");
      out = await translateGtxStr(str);
    }
    cache.set(str, out);
    return out;
...
async function processFile(name) {
  ...
  const plPath = path.join(DICT, "pl", name);
  const cpPath = checkpointPathFor(plPath);
  ...
  let plObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(plPath)) {
    try {
      plObj = JSON.parse(fs.readFileSync(plPath, "utf8"));
    } catch {
      plObj = {};
    }
  }
  ...
  for (const key of topKeys) {
    ...
    plObj[key] = await mapLeaves(enObj[key], cache);
    fs.writeFileSync(plPath, JSON.stringify(plObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", plPath, "after", key, "| cache size:", cache.size);
  }
