/**
 * tr/*.json → kk/*.json (құрылым сақталады).
 *
 * Қолдану:
 *   node scripts/i18n-tr-to-kk.mjs
 *   node scripts/i18n-tr-to-kk.mjs --file solutions
 *   node scripts/i18n-tr-to-kk.mjs --file solutions --fresh
 *   node scripts/i18n-tr-to-kk.mjs --file solutions --keys atexPatlamaKoruma,cfdDanismanlik
 *     (тек көрсетілген tr кілттерін kk-ға аударып, қалғанын сақтайды; --fresh жоқ)
 *   Қалған әдепкі файлдар (жеке іске қосу): technical, services, home, common, contact, sustainability
 *     мысалы: node scripts/i18n-tr-to-kk.mjs --file technical --fresh
 *
 * Орта айнымалылар:
 *   I18N_TR_TO_KK_ENGINE=gtx|mymemory|google|both   (әдепкі: gtx — translate.googleapis.com client=gtx, әдетте IP-бан аз)
 *   I18N_TR_TO_KK_GTX_DELAY_MS, I18N_TR_TO_KK_GTX_MAX (~1200) — GTX GET шегі
 *   I18N_TR_TO_KK_DELAY_MS, I18N_TR_TO_KK_COOLDOWN_MS — тек vitalets google үшін
 *   I18N_TR_TO_KK_MM_DELAY_MS — MyMemory
 *
 * MyMemory: ~500 таңба/сұрау; күндік лимит толса қате (тек mymemory режимінде тоқтау).
 *
 * Windows PowerShell: stderr пайпымен шығыс шектемеңіз (`| Select-Object -First N`) — Node тоқтауы мүмкін.
 * Жазыңыз: `node scripts/i18n-tr-to-kk.mjs` немесе `npm run i18n:tr-to-kk`.
 */
import fs from "fs";
...
const ENGINE = (process.env.I18N_TR_TO_KK_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_TR_TO_KK_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_TR_TO_KK_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_TR_TO_KK_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_TR_TO_KK_GTX_DELAY_MS) || 380;
const GTX_MAX = Number(process.env.I18N_TR_TO_KK_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
...
const DEFAULT_FILES = ["solutions.json", "products.json", "corporate.json"];
...
function fixBrand(s) {
  return s.replace(/\|\s*романдар\b/gi, "| Novves");
}
...
function checkpointPathFor(kkPath) {
  return kkPath.replace(/\.json$/i, "") + ".tr-to-kk.checkpoint.json";
}
...
/** Ұзын жолды MyMemory шегіне сыйғыза бөлу */
...
/** GTX GET ұзындығы шектеулі — ұзын жолдар бөлікке бөлінеді */
...
async function translateGtxOne(chunk) {
  let lastErr;
  const urlBase =
    "https://translate.googleapis.com/translate_a/single?client=gtx&sl=tr&tl=kk&dt=t&q=";
  for (let attempt = 0; attempt < GTX_RETRIES; attempt++) {
    ...
    try {
      ...
      lastErr = e;
      console.error("gtx retry", attempt + 1, "/", GTX_RETRIES, String(e?.message ?? e).slice(0, 100));
    }
  }
...
async function translateMyMemoryOne(chunk) {
  ...
  const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(chunk)}&langpair=tr|kk`;
...
async function translateGoogleStr(str, cache) {
  ...
  for (let attempt = 0; attempt < RETRIES; attempt++) {
    ...
    await sleep(wait);
    try {
      const res = await translate(str, { from: "tr", to: "kk" });
...
async function translateText(str, cache) {
  ...
  try {
    ...
    if (ENGINE === "gtx") {
      ...
      console.error("Unknown I18N_TR_TO_KK_ENGINE:", ENGINE, "— use gtx|mymemory|google|both");
      out = await translateGtxStr(str);
    }
    cache.set(str, out);
    return out;
    ...
    if (e?.fatalQuota || msg.includes(MYMEMORY_QUOTA_ERR)) {
      console.error("MyMemory quota — stopping to avoid writing untranslated Turkish strings. Retry later.");
...
async function processFile(name) {
  const trPath = path.join(DICT, "tr", name);
  const kkPath = path.join(DICT, "kk", name);
  const cpPath = checkpointPathFor(kkPath);
  ...
  let kkObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(kkPath)) {
    try {
      kkObj = JSON.parse(fs.readFileSync(kkPath, "utf8"));
    } catch {
      kkObj = {};
    }
  }
  ...
  if (ONLY_KEYS) {
    const missing = [...ONLY_KEYS].filter((k) => !(k in trObj));
    if (missing.length) console.error("Unknown --keys (not in tr):", missing.join(", "));
  }
  ...
  for (const key of topKeys) {
    ...
    kkObj[key] = await mapLeaves(trObj[key], cache);
    fs.writeFileSync(kkPath, JSON.stringify(kkObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", kkPath, "after", key, "| cache size:", cache.size);
  }
