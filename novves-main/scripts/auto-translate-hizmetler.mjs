...
const sourcePath = new URL("../src/lib/hizmetler-tr-to-en.json", import.meta.url);
const outPath = new URL("../src/lib/hizmetler-tr-to-locales.auto.json", import.meta.url);
...
async function main() {
  ...
  for (const [locale, target] of Object.entries(targetLocales)) {
    ...
    for (const key of keys) {
      try {
        if (locale === "en") {
          out[locale][key] = trToEn[key];
        } else {
          const translated = await translateText(key, target);
