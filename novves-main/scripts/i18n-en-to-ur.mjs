/**
 * en/*.json → ur/*.json (JSON structure preserved, leaf strings → Urdu).
 * Locale `ur` is used for Pakistan / اردو in this project.
 *
 * Usage:
 *   node scripts/i18n-en-to-ur.mjs
 *   node scripts/i18n-en-to-ur.mjs --file solutions --fresh
 *
 * Env:
 *   I18N_EN_TO_UR_ENGINE=gtx|mymemory|google|both   (default: gtx)
 *   I18N_EN_TO_UR_GTX_DELAY_MS, I18N_EN_TO_UR_GTX_MAX (~1200)
 *   I18N_EN_TO_UR_DELAY_MS, I18N_EN_TO_UR_COOLDOWN_MS
 *   I18N_EN_TO_UR_MM_DELAY_MS
 *
 * npm: npm run i18n:en-to-ur
 */
import fs from "fs";
...
const ENGINE = (process.env.I18N_EN_TO_UR_ENGINE || "gtx").toLowerCase();
const DELAY_MS = Number(process.env.I18N_EN_TO_UR_DELAY_MS) || 3200;
const COOLDOWN_ON_429_MS = Number(process.env.I18N_EN_TO_UR_COOLDOWN_MS) || 120_000;
const MYMEMORY_DELAY_MS = Number(process.env.I18N_EN_TO_UR_MM_DELAY_MS) || 450;
const MYMEMORY_MAX = 480;
const GTX_DELAY_MS = Number(process.env.I18N_EN_TO_UR_GTX_DELAY_MS) || 320;
const GTX_MAX = Number(process.env.I18N_EN_TO_UR_GTX_MAX) || 1200;
const GTX_RETRIES = 8;
...
function checkpointPathFor(urPath) {
  return urPath.replace(/\.json$/i, "") + ".en-to-ur.checkpoint.json";
}
...
async function translateGoogleStr(str, cache) {
  ...
  for (let attempt = 0; attempt < RETRIES; attempt++) {
    ...
    await sleep(wait);
    try {
      const res = await translate(str, { from: "en", to: "ur" });
...
async function translateText(str, cache) {
  ...
  try {
    ...
    if (ENGINE === "gtx") {
      ...
      console.error("Unknown I18N_EN_TO_UR_ENGINE:", ENGINE, "— use gtx|mymemory|google|both");
      out = await translateGtxStr(str);
    }
    cache.set(str, out);
    return out;
...
async function processFile(name) {
  ...
  const urPath = path.join(DICT, "ur", name);
  const cpPath = checkpointPathFor(urPath);
  ...
  let urObj = {};
  if ((!FRESH || ONLY_KEYS) && fs.existsSync(urPath)) {
    try {
      urObj = JSON.parse(fs.readFileSync(urPath, "utf8"));
    } catch {
      urObj = {};
    }
  }
  ...
  for (const key of topKeys) {
    ...
    urObj[key] = await mapLeaves(enObj[key], cache);
    fs.writeFileSync(urPath, JSON.stringify(urObj, null, 2) + "\n", "utf8");
    if (!ONLY_KEYS) {
      doneKeys.add(key);
      saveDoneKeys(cpPath, doneKeys);
    }
    console.error("Wrote", urPath, "after", key, "| cache size:", cache.size);
  }
