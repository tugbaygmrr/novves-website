...
const headerSchema = z.object({
  eyebrow: localeMap.default({}),
  title: localeMap.default({}),
  lead: localeMap.default({}),
  sidebarCardTitle: localeMap.default({}),
  sidebarCardDesc: localeMap.default({}),
  cardCta: localeMap.default({}),
  mainCta: localeMap.default({}),
});
const boxSchema = z.object({
  href: z.string().trim().min(1).max(300),
  icon: z.string().trim().max(60).default(""),
  iconImage: z.string().trim().max(500).default(""),
  image: z.string().trim().max(500).default(""),
  videoSrc: z.string().trim().max(500).default(""),
  videoPoster: z.string().trim().max(500).default(""),
  title: localeMap.default({}),
  description: localeMap.default({}),
});
const bodySchema = z.object({
  header: headerSchema,
  boxes: z.array(boxSchema).max(12),
});
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ header: {}, boxes: [] });
  }
}
