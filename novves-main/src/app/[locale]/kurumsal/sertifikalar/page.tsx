...
export default async function Sertifikalar({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main>
      <section className="relative flex flex-col overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-0 pt-14 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 max-w-3xl pb-20">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 backdrop-blur-sm"><span className="h-1.5 w-1.5 rounded-full bg-primary" /><span className="text-xs font-semibold uppercase tracking-wider text-primary">{t.badge}</span></div>
      ...
      <section className="border-b border-secondary/10 bg-light">
          ...
          <div className="space-y-16">
            {t.groups.map((group: { title: string; items: { name: string; code?: string; description?: string; pdf: string }[] }, groupIdx: number) => (
              <div key={group.title} className="relative">
                {groupIdx > 0 && (<div className="absolute -top-8 left-0 right-0 h-px bg-gradient-to-r from-transparent via-secondary/10 to-transparent" />)}
                <div className="mb-8 flex items-center gap-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-secondary text-white">{groupIcons[groupIdx] || groupIcons[0]}</div>
                  <div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {group.items.map((cert) => (
                    <a key={cert.pdf} href={cert.pdf} target="_blank" rel="noopener noreferrer" className="group relative flex overflow-hidden rounded-lg border border-secondary/8 bg-white transition-all duration-200 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5">
                      <div className="w-1 flex-shrink-0 bg-secondary/10 transition-colors duration-200 group-hover:bg-primary" />
                      <div className="flex flex-1 items-start gap-4 p-5">
                        <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-lg bg-secondary/5 transition-colors duration-200 group-hover:bg-primary/10">
