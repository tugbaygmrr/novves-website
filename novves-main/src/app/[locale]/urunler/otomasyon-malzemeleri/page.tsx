...
export async function generateMetadata({
  ...
  return productCategoryMetadata(locale, "otomasyonMalzemeleri");
}
  ...
  return (
    <main>
      ...
      <section className="relative overflow-hidden bg-[#ecebe6]">
        ...
        <span className="pointer-events-none absolute right-0 top-0 select-none text-[20rem] font-black leading-none text-[#d8d5cc]/45" aria-hidden="true">04</span>
        <div className="relative mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
          <div className="mb-14 flex items-end gap-6">
            <div>
              ...
              <h2 className="mt-2 font-eurostile text-card font-extrabold tracking-tight text-dark">{s.productGroups}</h2>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {t.products.map((product: { name: string; subModels: string[] }, i: number) => (
              <div key={product.name} className="group relative overflow-hidden rounded-xl border border-ink/10 bg-[#f5f2eb] p-6 transition-all duration-300 hover:bg-[#f8f5ed] hover:shadow-[0_16px_30px_-22px_rgba(15,20,30,0.3)] hover:border-primary/25">
                ...
                <span className="mb-3 block text-3xl font-black text-[#c8c7c2]/50">{String(i + 1).padStart(2, "0")}</span>
                <h3 className="font-eurostile text-card-sm font-bold text-dark">{product.name}</h3>
                {product.subModels.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {product.subModels.map((m: string) => (
                      <span key={m} className="rounded-lg border border-ink/10 bg-[#fbf9f3] px-3 py-1.5 text-xs font-semibold text-secondary/60">{m}</span>
                    ))}
                  </div>
                )}
              </div>
