        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mb-10 flex items-end gap-6 sm:mb-12">
            <div>
              ...
              <h2 className="mt-2 font-eurostile text-section font-extrabold tracking-tight text-dark">ROO</h2>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {t.roo.subModels.map((model: string, i: number) => (
              <div key={model} className="group relative overflow-hidden rounded-xl border border-ink/10 bg-[#f5f2eb] px-5 py-4 shadow-[0_10px_24px_-24px_rgba(15,20,30,0.22)] transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/25 hover:bg-[#f8f5ed] hover:shadow-[0_16px_30px_-22px_rgba(15,20,30,0.3)]">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-xl font-black text-secondary/[0.14]">{String(i + 1).padStart(2, "0")}</span>
          ...
          <div className="mt-12 flex flex-wrap gap-3">
            <Link href={`/${locale}/iletisim`} className="btn-3d btn-3d-dark group inline-flex items-center gap-3 rounded-xl border border-[#1d2f4d]/25 bg-[#1d2f4d] px-7 py-4 text-sm font-bold text-white transition-all duration-300 hover:border-primary hover:bg-primary hover:shadow-xl hover:shadow-primary/25">
            ...
            <a href="tel:+902164674752" className="inline-flex items-center gap-2 rounded-xl border border-ink/12 bg-[#f7f4ed] px-7 py-4 text-sm font-bold text-secondary/82 transition-all duration-300 hover:border-primary/30 hover:text-primary">
              +90 216 467 47 52
            </a>
          </div>
        </div>
            ...
            {t.otherCategories.map((cat: { label: string; slug: string }, i: number) => (
              <Link key={cat.label} href={`/${locale}/urunler/${cat.slug}`} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.06] p-6 shadow-[0_18px_42px_-34px_rgba(8,14,24,0.7)] backdrop-blur-sm transition-all duration-500 hover:-translate-y-1 hover:border-primary/35 hover:bg-white/[0.12] hover:shadow-[0_24px_50px_-28px_rgba(239, 95, 23,0.22)]">
                ...
                <div className="mt-5 flex items-center gap-2">
                  ...
                  <svg className="h-3.5 w-3.5 text-primary/65 transition-all duration-300 group-hover:translate-x-0.5 group-hover:text-primary" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>
                </div>
